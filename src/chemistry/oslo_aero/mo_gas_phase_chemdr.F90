...
!
       ...
       !!!change by rt!!!
       integer :: idx_DMS,ncdate1,ncsec1,yr1,tod1,mon1,day1,i_rt
       real(r8) :: vmr_prod_out_soa_lv_from_dms(ncol,pver)
       real(r8) :: vmr_prod_out_soa_sv_from_dms(ncol,pver)
       real(r8) :: mmr_prod_out_soa_lv_from_dms(ncol,pver)
       real(r8) :: mmr_prod_out_soa_sv_from_dms(ncol,pver)
       real(r8) :: mmr_dms_mo_gas_phase(ncol,pver)
       real(r8) :: vmr_dms_mo_gas_phase(ncol,pver)
       real(r8) :: mmr_soa_lv(ncol,pver)
       real(r8) :: vmr_soa_lv(ncol,pver)
       real(r8) :: mmr_soa_sv(ncol,pver)
       real(r8) :: vmr_soa_sv(ncol,pver)

       !!!end change by rt!!!

       !!!change by rt!!!
       idx_DMS = get_spc_ndx('DMS')
       call get_curr_date (yr1,mon1,day1,ncsec1)
       ncdate1 = yr1*10000 + mon1*100 + day1
       vmr_prod_out_soa_lv_from_dms = 0.
       vmr_prod_out_soa_sv_from_dms = 0.
       mmr_prod_out_soa_lv_from_dms = 0.
       mmr_prod_out_soa_sv_from_dms = 0.
       mmr_dms_mo_gas_phase = 0.
       vmr_dms_mo_gas_phase = 0.
       vmr_soa_lv = 0.
       mmr_soa_lv = 0.
       vmr_soa_sv = 0.
       mmr_soa_sv = 0.
       ...
       !!!change by rt!!!
       n = map2chm(12) !!!dms index in chemistry
       mmr_dms_mo_gas_phase(:ncol,:)  = mmr(:ncol,:,m)
       vmr_dms_mo_gas_phase(:ncol,:)  = vmr(:ncol,:,m)
...
!
       ...
       stratochem: if ( has_strato_chem ) then
           ...
           !        ... call SAD routine
           !-----------------------------------------------------------------------      
           write(iulog,*) 'reaction rate ', rxntot
...
!
       ...
       call imp_sol( vmr, reaction_rates, het_rates, extfrc, delt, &
                                   ...
                                   vmr_prod_out_soa_lv_from_dms,vmr_prod_out_soa_sv_from_dms )
       !if(lchnk == 888) then
       do i_rt = 1,ncol                                    
           do k = 1,pver
               if( adv_mass(26) /= 0._r8 ) then
                   mmr_prod_out_soa_lv_from_dms(i_rt,k) = adv_mass(26)*vmr_prod_out_soa_lv_from_dms(i_rt,k) / mbar(i_rt,k)
               endif
               if( adv_mass(27) /= 0._r8 ) then
                   mmr_prod_out_soa_sv_from_dms(i_rt,k) = adv_mass(27)*vmr_prod_out_soa_sv_from_dms(i_rt,k) / mbar(i_rt,k)
               endif
           enddo
       enddo
       !endif
...
!
       ...
       !!!change by rt!!!
       vmr_soa_lv = vmr(:ncol,:,26)
       mmr_soa_lv = mmr(:ncol,:,26)
       vmr_soa_sv = vmr(:ncol,:,27)
       mmr_soa_sv = mmr(:ncol,:,27)
       !!!end change by rt!!!
       ...
       call chm_diags( lchnk, ncol, vmr(:ncol,:,:), mmr_new(:ncol,:,:), &
                                       ...
                                       mmr_prod_out_soa_lv_from_dms,mmr_prod_out_soa_sv_from_dms,mmr_dms_mo_gas_phase,&
                                       vmr_prod_out_soa_lv_from_dms,vmr_prod_out_soa_sv_from_dms,vmr_dms_mo_gas_phase,&
                                       vmr_soa_lv,mmr_soa_lv,vmr_soa_sv,mmr_soa_sv )
