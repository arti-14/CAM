module mo_imp_sol
   ...
   !-----------------------------------------------------------------------
   ! Newton-Raphson iteration limitsimp_prod_loss
   !-----------------------------------------------------------------------
 ...
 contains
   ...
   subroutine imp_sol( base_sol, reaction_rates, het_rates, extfrc, delt, &
                       ncol,nlev, lchnk, prod_out, loss_out,prod_out_soa_lv_from_dms,prod_out_soa_sv_from_dms )
     !-----------------------------------------------------------------------
     ...
     !!!change by rt!!!
     real(r8), intent(out) :: prod_out_soa_lv_from_dms(ncol,nlev)
     real(r8), intent(out) :: prod_out_soa_sv_from_dms(ncol,nlev)
     !!!end change by rt!!!
     !-----------------------------------------------------------------------
     ! ... local variables
     !-----------------------------------------------------------------------
     ...
     !!!change by rt!!!
      real(r8) ::prod_soa_lv_from_dms, prod_soa_sv_from_dms
      !!end change by rt!!!
 
      !!!change by rt!!!
      prod_soa_lv_from_dms = 0. !!!units = units of base_sol/dt
      prod_soa_sv_from_dms = 0.!!!units = units of base_sol/dt
      prod_out_soa_lv_from_dms = 0.!!!units = units of base_sol
      prod_out_soa_sv_from_dms = 0.!!!units = units of base_sol
      prod_out = 0.
      loss_out = 0.
      !!!end change by rt!!!
     ...
     level_loop : do lev = 1,nlev
        column_loop : do i = 1,ncol
           ...
           cls_loop: do k = 1,clscnt4
              ...
              prod_out_soa_lv_from_dms(i,lev)= prod_soa_lv_from_dms*dt
              prod_out_soa_sv_from_dms(i,lev)= prod_soa_sv_from_dms*dt
           end do cls_loop
        end do column_loop
     end do level_loop
