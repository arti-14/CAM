module clubb_intr
 ...
 !updraft emulator vars
    ...
    real(r8) :: coszrs(pcols),cosz(pcols)   ! Cosine solar zenith angle
    ...
    REAL(dp):: pint_emu(pcols,pver),pmid_emu(pcols,pverp)
 #endif
    det_s(:)   = 0.0_r8
    det_ice(:) = 0.0_r8
 #ifdef CLUBB_SGS
  ...
  !  Copy the state to state1 array to use in this routine
    ...
    pint_emu = state1%pint
    pmid_emu = state1%pmid
    pmid_emu(:,pverp) = state1%ps(:)
    !call get_coszrs(cosz)
    call emulator(1,pcols, pcols, 1, pverp, pver, &
                   pint_emu(1:pcols,:), pmid_emu(1:pcols,:), &
                   state1%q(1:pcols,:,ixcldliq),state1%q(:pcols,:,ixcldice),state1%q(:pcols,:,ixq),&
                   cloud_frac(:pcols,1:pver),cam_in%shf(:ncol), coszrs(1:pcols), th(:pcols,:),&
                   pwsigma(:pcols,:), pemu_mask(:pcols), pemu_cb(:pcols,:))
