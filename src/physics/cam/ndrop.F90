module ndrop
   ...
   real(r8), allocatable :: f1_ghosh(:)          ! ghosh functions of width
   real(r8), allocatable :: f2_ghosh(:)          ! ghosh functions of width
   ...
   subroutine maxsat(zeta,eta,nmode,smc,smax)
      ...
      real(r8) :: p_ghosh  ! p from Ghosh et al. 2025
      do m=1,nmode
      ...
      do m=1,nmode
         if(eta(m).gt.1.e-20_r8)then
            ...
            if (g1 <= 1.0_r8) then
               ...
               !Ghosh et al. 2025
               p_ghosh = -0.5073_r8 + (1.5088_r8*sigmag_amode(m)) - (0.3699_r8*sigmag_amode(m)*sigmag_amode(m))
               f1_ghosh(m)          = 0.0135_r8*exp(2.367_r8*sigmag_amode(m))
               f2_ghosh(m)          =1.1058_r8 - (0.315_r8*sigmag_amode(m))
               g1 = g1 ** p_ghosh
               sum=sum+(f1_ghosh(m)*g1+f2_ghosh(m)*g2)/(smc(m)*smc(m))
            endif
         else
            sum=1.e20_r8
         endif
